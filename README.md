...
- 支持当前烘焙草稿自动保存：刷新、误关页面后可恢复当前曲线与事件记录。
- WiFi/WebSocket 连接会记住上次成功连接的设备 IP，后续可直接重连。
...
- ET 自动显示策略：
  - 若设备未稳定提供 ET，界面会自动隐藏 ET 与 ET RoR（避免误判）
  - 若 ET 信号恢复并持续有效，会自动恢复显示
- 图表坐标轴优化：
...
- 图表内事件显示优化：
  - 关键事件（如入豆/一爆/二爆/下豆）高亮显示
  - 事件标签采用上下“双车道”分流，降低重叠
  - 非关键事件弱化显示并保留事件点，减少遮挡曲线
...
- 烘焙进行中会自动把当前记录保存到浏览器本地
- 保存内容包括曲线数据、事件标记、开始时间、当前 BT/ET/RoR 与采样间隔
- 刷新页面、误关页面或重新打开应用后，会自动恢复当前烘焙草稿
- 重新连接设备后，会继续追加到同一条曲线，不会从头开始记录
- 手动“重置”或“清除目前曲线”会清除当前草稿
...
点击界面中的 `WiFi` 按钮后，首次输入设备 IP（只输入 IP 即可），应用会固定拼接成：
...
首次成功连接后，应用会把设备 IP 保存在浏览器本地；以后点击 `WiFi` 会直接使用上次 IP 连接，不需要重复输入。

如果上次保存的 IP 连接失败，应用会清除旧 IP 并弹出输入框，让你输入新的设备地址。
...
说明：若设备仅提供 BT，应用会将 ET 视为“未稳定提供”，不会强行显示 ET 曲线与 ET RoR。
